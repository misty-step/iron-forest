...
`
	...
	select {
	case result := <-done:
		if result.record.Exit != runCancelledExit || result.record.Outcome != runOutcomeCancelled || result.record.ProcessExit == nil || *result.record.ProcessExit != -1 || !errors.Is(result.err, errRunCancelled) || errors.Is(result.err, errRunTimedOut) {
	...
	if rows[0].RunID != runID || rows[0].Exit != runCancelledExit || rows[0].Outcome != runOutcomeCancelled || rows[0].ProcessExit == nil || *rows[0].ProcessExit != -1 {
