...
func TestPublishReviewRequestUsesTrustedSecretsScanner(t *testing.T) {
	...
	// The candidate carries a neutered scanner and still declares the old
	// candidate-supplied command. The Gate must run its own scanner and find
	// the planted fixture rather than compiling the candidate's scansecrets.go.
...
checks:
  - {name: test, run: "true"}
  - {name: secrets, run: "go run . scan-secrets ."}
`)
	if err := os.WriteFile(filepath.Join(root, "forest.yaml"), config, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), `check "secrets" failed`) || !strings.Contains(err.Error(), "fixture.txt") {
		t.Fatalf("error=%v, want trusted secrets check to fail on the planted fixture", err)
	}
