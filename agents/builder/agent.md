...
For every note actor check, set `note_path="${revision:0:2}/${revision:2}"` from the exact target `revision` and use `git log -1 --format='%an <%ae>' <ref> -- "$note_path"`. Never search by blob.
