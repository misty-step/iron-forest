...
not republished. The merge never uses force. The `secrets` Check is run by the
Gate's own scanner (`$FOREST_ROOT/forest scan-secrets <worktree>`) against the
detached candidate worktree; it never compiles or executes candidate code, so
a candidate cannot supply the Gate's credential scanner. Except for the trusted
first `master` baseline, the Auditor checks the observable final state after
the Effect; it does not enforce it. See
