...
func TestPollSupervisorCancellationDrainsPrivateSnapshotCleanup(t *testing.T) {
	...
	if os.Getenv(helper) == "1" {
		...
		os.Exit(runCLI([]string{"poll", "verifier"}))
	}
	if directPollTimeout != 60*time.Second || pollCommandTimeout-directPollTimeout != 5*time.Second || pollStopGrace != 5*time.Second {
		t.Fatalf("Poll bounds direct=%s supervisor=%s grace=%s", directPollTimeout, pollCommandTimeout, pollStopGrace)
	}
...
set -eu
case " $* " in
  *" notes --ref=refs/notes/forest-poll/"*"/review-request show "*)
    trap '' HUP INT TERM
    printf '%s\n' "$$" > "$TRANSPORT_PID"
    printf x > "$SNAPSHOT_READY"
    while :; do printf x >> "$TRANSPORT_HEARTBEAT"; /bin/sleep 0.02; done
    ;;
  *" update-ref -d refs/notes/forest-poll/"*"/verdict "*)
    "$REAL_GIT" "$@"
    trap '' HUP INT TERM
    printf '%s\n' "$$" > "$CLEANUP_PID"
    while :; do printf x >> "$CLEANUP_HEARTBEAT"; /bin/sleep 0.02; done
    ;;
esac
exec "$REAL_GIT" "$@"
`
	...
	transportState, transportHeartbeat := processHeartbeatFixture(t)
	cleanupState, cleanupHeartbeat := processHeartbeatFixture(t)
	forestPIDPath := filepath.Join(transportState, "forest-pid")
	transportPIDPath := filepath.Join(transportState, "child-pid")
	cleanupPIDPath := filepath.Join(cleanupState, "child-pid")
	snapshotReady := filepath.Join(transportState, "snapshot-ready")
	t.Setenv("FOREST_POLL_PID", forestPIDPath)
	t.Setenv("TRANSPORT_PID", transportPIDPath)
	t.Setenv("TRANSPORT_HEARTBEAT", transportHeartbeat)
	t.Setenv("CLEANUP_PID", cleanupPIDPath)
	t.Setenv("CLEANUP_HEARTBEAT", cleanupHeartbeat)
	t.Setenv("SNAPSHOT_READY", snapshotReady)
	t.Setenv(helper, "1")

	ctx, cancel := context.WithTimeout(context.Background(), 8*time.Second)
	done := make(chan PollResult, 1)
	command := "cd " + quoteShellPath(root) + " && " + quoteShellPath(os.Args[0]) + " -test.run=^TestPollSupervisorCancellationDrainsPrivateSnapshotCleanup$"
	...
	waitForCLIFile(t, forestPIDPath)
	waitForCLIFile(t, transportPIDPath)
	waitForCLIFile(t, transportHeartbeat)
	waitForCLIFile(t, snapshotReady)
	if refs := strings.Fields(string(runGitDir(t, root, "for-each-ref", "--format=%(refname)", pollNotesNamespace+"/"))); len(refs) != 1 {
		t.Fatalf("private Poll refs at cancellation=%v, want one fetched review snapshot", refs)
	}
	readPID := func(path string) int {
		...
		if err != nil {
			t.Fatalf("process pid %q: %v", body, err)
		}
		return pid
	}
	forestPID, transportPID := readPID(forestPIDPath), readPID(transportPIDPath)
	forestGroup, err := syscall.Getpgid(forestPID)
	if err != nil {
		t.Fatal(err)
	}
	transportGroup, err := syscall.Getpgid(transportPID)
	if err != nil {
		t.Fatal(err)
	}
	if transportGroup != transportPID || transportGroup == forestGroup {
		t.Fatalf("process groups forest=%d transport=%d transport_pid=%d", forestGroup, transportGroup, transportPID)
	}

	cancel()
	waitForCLIFile(t, cleanupPIDPath)
	waitForCLIFile(t, cleanupHeartbeat)
	cleanupPID := readPID(cleanupPIDPath)
	cleanupGroup, err := syscall.Getpgid(cleanupPID)
	if err != nil {
		t.Fatal(err)
	}
	if cleanupGroup != cleanupPID || cleanupGroup == forestGroup || cleanupGroup == transportGroup {
		t.Fatalf("process groups forest=%d transport=%d cleanup=%d cleanup_pid=%d", forestGroup, transportGroup, cleanupGroup, cleanupPID)
	}
	...
	case result = <-done:
		waited = true
	case <-time.After(pollStopGrace + time.Second):
		t.Fatal("supervisor cancellation did not return after the full Poll cleanup allowance")
	}
	if result.Code != 2 || !errors.Is(result.Err, context.Canceled) {
		t.Fatalf("canceled direct Poll result=%#v", result)
	}
	if !waitProcessGroupQuiescence(transportPID, time.Second) {
		t.Fatalf("Git Poll transport group %d survived supervisor cancellation", transportPID)
	}
	if !waitProcessGroupQuiescence(cleanupPID, time.Second) {
		t.Fatalf("Git Poll cleanup group %d survived supervisor cancellation", cleanupPID)
	}
	assertProcessQuiescent(t, transportHeartbeat, "Git Poll transport group", "supervisor cancellation")
	assertProcessQuiescent(t, cleanupHeartbeat, "Git Poll cleanup group", "fresh cleanup deadline")
	if refs := strings.TrimSpace(string(runGitDir(t, root, "for-each-ref", "--format=%(refname)", pollNotesNamespace+"/"))); refs != "" {
		t.Fatalf("private Poll refs survived supervisor cancellation: %s", refs)
	}
}
