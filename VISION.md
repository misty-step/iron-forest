...
| Revision | One commit SHA. Every note binds to it. |
...
| Gate | One valid review-request, passing Checks, an approve Verdict, and a fast-forward of `master` to that Revision. |
| Auditor | A read-only check of observable Git state after a dispatch. It does not merge. |
...
forest:ready Issue
  ...
  → Verifier checks, judges, writes Checks and Verdict, maybe merges
  → Fixer repairs a rejected Revision
```

Git holds durable facts: branches, commits, and `refs/notes/forest/*`. The
Ledger and Run logs are local to the instance. They are not a second work
store.
...
Raw `git notes` is the note escape. The Kernel does not wrap note inspection.
